...
let section = Uwt_log.Section.make "lwt(timeout)"
...
let handle_exn =
  ref
    (fun exn ->
       ignore (Uwt_log.error ~section ~exn "uncaught exception after timeout");
       exit 1)
...
let create delay action =
  if delay < 1 then invalid_arg "Uwt_timeout.create";
...
let change x delay =
  if delay < 1 then invalid_arg "Uwt_timeout.change";
  x.delay <- delay;
  size delay;
